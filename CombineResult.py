...
def combineReuslt(folder_name):
    ...
    cmd = "for /r {0} %i in (*result.json) do @echo %i".format(
        folder_name) if system_info == 'Windows' else "find {0}/* -name 'result.json'".format(folder_name)
        ...
        with open("all_result.txt", 'w') as f:
            ...
            for url, value_list in all_result_dict.items():
                ...
                if value_list['email']:
                    ...
                    ### 新增内容开始 ###
                if value_list['sensitive_words']:
                    f.write("sensitive words evidence: {}\n".format(",".join(value_list['sensitive_words'])))
                ### 新增内容结束 ###
                f.write("-------------------\n")
